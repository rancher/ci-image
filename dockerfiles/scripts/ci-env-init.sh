...
#
# Scans for SELECT_{FAMILY}_VERSION environment variables and automatically runs
...
# Auto-configure families based on SELECT_{FAMILY}_VERSION env vars
if [ -d "${FAMILIES_DIR}" ]; then
    for _family_dir in "${FAMILIES_DIR}"/*/; do
        ...
        # e.g., "helm" -> "SELECT_HELM_VERSION"
        _env_var=SELECT_$(printf '%s' "${_family}" | tr '[:lower:]-' '[:upper:]_')_VERSION
